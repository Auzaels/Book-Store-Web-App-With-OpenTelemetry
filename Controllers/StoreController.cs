...
namespace BookStore.Controllers
{
    ...
    public class StoreController : Controller
    {
        ...
        public async Task<IActionResult> Index(string searchString, string minPrice, string maxPrice)
        {
            ...
            {
                books = books.Where(b => b.BookTitle.Contains(searchString) || b.BookAuthor.Contains(searchString));
            }

            if (!string.IsNullOrEmpty(minPrice))
            {
                var min = int.Parse(minPrice);
                books = books.Where(b => b.BookPrice >= min);
            }

            if (!string.IsNullOrEmpty(maxPrice))
            {
                var max = int.Parse(maxPrice);
                books = books.Where(b => b.BookPrice <= max);
            }
